(function ($) {
    ...
    // validation of the formulary
        ...
        // function for make disapear the footer link when we are on the bottom of the page
        ...
        // toggle hamburger
        const hanburger = document.querySelector('.icon-hamburger');
        const nav = document.querySelector('.ualp-nav-link2');
        const cross = document.querySelector('.fa-solid.fa-xmark')
        ...
        cross.addEventListener('click', () => {
            ...
            // Remove the active class after the animation duration (e.g., 0.5 seconds)
            setTimeout(() => {
              nav.classList.remove('active');
              // Remove the closing class to reset the opacity and display properties
              nav.classList.remove('closing');
            }, 500); // Adjust the animation duration as needed
        });

        // change border of the form
        let go_to_form = document.querySelectorAll('.go-to-form');
        let form = document.querySelector('.omnes_form');
        go_to_form.forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            form.classList.add('changeBorderColorDuringThreeSeconds');
            setTimeout(() => {
                form.classList.remove('changeBorderColorDuringThreeSeconds');
            }, 3700);
        });
    });
